...
JA_ZH_UTF8_JSON = '../data/ja_zh.json'
SINKAI_EQUIP_ID_BASE = 501
EQUIPS2_LUA = '../lua/equips2.lua'
EQUIPS2_JSON = '../json/equips2.json'
...
def shinkai_parse_equip(start2):
    ...
    with open(JA_ZH_UTF8_JSON, 'r', encoding='utf-8') as json_fp:
...
def shinkai_generate_equip_json(start2):
    ...
    equips = [i for i in start2['api_mst_slotitem']
              if i['api_id'] >= SINKAI_EQUIP_ID_BASE]
    with open(EQUIPS2_JSON, 'w', encoding='utf8') as json_fp:
...
def shinkai_generate_equip_lua(start2):
    ...
    with open(EQUIPS2_LUA, 'w', encoding='utf8') as lua_fp:
